...
This is a Node.js 24 GitHub Action. The action's entry point is `dist/main.js`, produced by tsup bundling `src/main.ts` into a single ESM file. The `dist/` folder must be committed — CI verifies there is no git diff after building.
